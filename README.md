...
合规托管等公司类型，**每 5 小时自动同步职位**（每日多次）。包含职位列表、数据洞察、合作单位（公司库）三个页面，均按职能 / 公司类型 / 级别 / 地区维度组织。页面顶部显示精确到分钟的最近更新时间。
...
| `companies.html` | 合作单位（公司库）页。各招聘公司简介 + 基于在招职位实时计算的分析（在招数、薪资中位、远程占比、主要职能、最新发布），点击直达该公司职位。公司简介存于 `data.js` 的 `companies` 字段。 |
